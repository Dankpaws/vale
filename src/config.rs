...
/// README.md and the installation documentation.
...
pub struct Config {
	...
	#[serde(rename = "REDLIB_DEFAULT_COLLAPSE_CHILD_COMMENTS")]
	#[serde(alias = "LIBREDDIT_DEFAULT_COLLAPSE_CHILD_COMMENTS")]
	pub(crate) default_collapse_child_comments: Option<String>,
	...
	#[serde(rename = "VALE_PROFILE_MODE")]
	pub(crate) profile_mode: Option<String>,

	#[serde(rename = "VALE_PROFILE_DATABASE")]
	#[serde(skip_serializing)]
	pub(crate) profile_database: Option<String>,

	#[serde(rename = "VALE_SESSION_DAYS")]
	pub(crate) session_days: Option<String>,

	#[serde(rename = "VALE_COOKIE_SECURE")]
	pub(crate) cookie_secure: Option<String>,

	#[serde(rename = "VALE_ARCHIVE_DIR")]
	#[serde(skip_serializing)]
	pub(crate) archive_dir: Option<String>,

	#[serde(rename = "VALE_ARCHIVE_ITEM_MAX_BYTES")]
	pub(crate) archive_item_max_bytes: Option<String>,

	#[serde(rename = "VALE_ARCHIVE_TOTAL_MAX_BYTES")]
	pub(crate) archive_total_max_bytes: Option<String>,
}
...
impl Config {
	...
	pub fn load() -> Self {
		...
		Self {
			...
			default_collapse_child_comments: parse("REDLIB_DEFAULT_COLLAPSE_CHILD_COMMENTS"),
			...
			profile_mode: parse("VALE_PROFILE_MODE"),
			profile_database: parse("VALE_PROFILE_DATABASE"),
			session_days: parse("VALE_SESSION_DAYS"),
			cookie_secure: parse("VALE_COOKIE_SECURE"),
			archive_dir: parse("VALE_ARCHIVE_DIR"),
			archive_item_max_bytes: parse("VALE_ARCHIVE_ITEM_MAX_BYTES"),
			archive_total_max_bytes: parse("VALE_ARCHIVE_TOTAL_MAX_BYTES"),
		}
	}
}
...
fn get_setting_from_config(name: &str, config: &Config) -> Option<String> {
	match name {
		...
		"REDLIB_DEFAULT_COLLAPSE_CHILD_COMMENTS" => config.default_collapse_child_comments.clone(),
		...
		"VALE_PROFILE_MODE" => config.profile_mode.clone(),
		"VALE_PROFILE_DATABASE" => config.profile_database.clone(),
		"VALE_SESSION_DAYS" => config.session_days.clone(),
		"VALE_COOKIE_SECURE" => config.cookie_secure.clone(),
		"VALE_ARCHIVE_DIR" => config.archive_dir.clone(),
		"VALE_ARCHIVE_ITEM_MAX_BYTES" => config.archive_item_max_bytes.clone(),
		"VALE_ARCHIVE_TOTAL_MAX_BYTES" => config.archive_total_max_bytes.clone(),
		_ => None,
	}
}
...
"#;
		write("redlib.toml", config_to_write).unwrap();
		assert_eq!(get_setting("VALE_ARCHIVE_DIR"), Some("/var/lib/vale/archives".into()));
		assert_eq!(get_setting("VALE_ARCHIVE_ITEM_MAX_BYTES"), Some("1073741824".into()));
		assert_eq!(get_setting("VALE_ARCHIVE_TOTAL_MAX_BYTES"), Some("2147483648".into()));
	}

	#[test]
	fn serialized_config_omits_private_filesystem_paths() {
		let config = Config {
			profile_database: Some("/home/reader/private/profiles.sqlite3".into()),
			archive_dir: Some("/home/reader/private/archives".into()),
			..Config::default()
		};
		let serialized = serde_json::to_string(&config).unwrap();
		assert!(!serialized.contains("VALE_PROFILE_DATABASE"));
		assert!(!serialized.contains("VALE_ARCHIVE_DIR"));
		assert!(!serialized.contains("/home/reader"));
	}
	...
	#[test]
	#[sealed_test(env = [("REDLIB_DEFAULT_COLLAPSE_CHILD_COMMENTS", "on")])]
	fn test_default_collapse_child_comments() {
		assert_eq!(get_setting("REDLIB_DEFAULT_COLLAPSE_CHILD_COMMENTS"), Some("on".into()));
	}
