...
use crate::{config, dbg_msg, utils::cookie_is_secure};
...
impl Server {
	...
	pub fn listen(self, addr: &str) -> Boxed<Result<(), hyper::Error>> {
		let make_svc = make_service_fn(move |_conn| {
			...
			// `service_fn` converts the handler closure into the Hyper service for
			// this connection.
			async move {
				Ok::<_, String>(service_fn(move |mut req: Request<Body>| {
					let req_headers = req.headers().clone();
					let def_headers = default_headers.clone();
					if req.uri().path_and_query().map_or(1, |value| value.as_str().len()) > MAX_REQUEST_TARGET_BYTES {
						return new_boilerplate(def_headers, req_headers, 414, Body::from("Request target is too large")).boxed();
					}
					let cookie_bytes = req_headers
						.get_all(header::COOKIE)
						.iter()
						.fold(0usize, |total, value| total.saturating_add(value.as_bytes().len()));
					if cookie_bytes > MAX_COOKIE_HEADER_BYTES {
						return new_boilerplate(def_headers, req_headers, 431, Body::from("Cookie headers are too large")).boxed();
					}
					...
					match crate::account::prepare_request(&mut req) {
						Ok(Some(mut response)) => {
							return async move {
								response.headers_mut().extend(def_headers);
								let _ = compress_response(&req_headers, &mut response).await;
								Ok(response)
							}
							.boxed();
						}
						Err(message) => return new_boilerplate(def_headers, req_headers, 500, Body::from(message)).boxed(),
						Ok(None) => {}
					}
					...
					match router.recognize(&format!("/{}{}", method.as_str(), path)) {
						...
						Ok(found) => {
							let response_cache_control = crate::account::response_cache_control(&path);
								...
								match func.await {
									Ok(mut res) => {
										...
										if let Some(cache_control) = response_cache_control {
											res.headers_mut().insert(header::CACHE_CONTROL, header::HeaderValue::from_static(cache_control));
										}
										if is_head {
...
async fn compress_response(req_headers: &HeaderMap<header::HeaderValue>, res: &mut Response<Body>) -> Result<(), String> {
	...
	if let Some(hdr) = res.headers().get(header::CONTENT_TYPE) {
		match from_utf8(hdr.as_bytes()) {
			Ok(val) => {
				...
				// Text and JSON are the only buffered response families Vale compresses;
				// media and other response types stay on their streaming paths.
	...
	// headers).
	let size_hint = res.body().size_hint();
	if size_hint.lower() < 1452 {
		return Ok(());
	};
	let Some(upper) = size_hint.upper() else {
		return Ok(());
	};
	if upper > MAX_COMPRESSIBLE_RESPONSE_BYTES {
		return Ok(());
	}
...
mod tests {
	...
	async fn test_compress_response() {
		...
		for accept_encoding in [
			...
			// Build test response.
			let lorem_ipsum = "Vale compression test payload. ".repeat(500);
			...
			let mut decoder: Box<dyn io::Read> = match expected_encoding {
				CompressionType::Gzip => Box::new(GzDecoder::new(&mut body_cursor)),
