...
pub async fn item(req: Request<Body>) -> Result<Response<Body>, String> {
	...
	match json(path, quarantined).await {
		...
		Ok(response) => {
			...
			// Reddit's duplicate-listing API omits cursor fields needed for normal
			// bidirectional pagination, so derive them from the boundary posts.
			//
			...
			if l > 0 {
				...
				// Address the missing "after" value returned for "before=" requests.
				if have_before {
